...
angular.module('ControladorInformaciones', [])
    .controller('InfAgenciasCobroCtrl', function ($scope, $ionicActionSheet, $timeout, $rootScope, ServiciosRuat, ServicioDatos, $location, $ionicLoading, $ionicPopup, $ionicHistory, $state, $cordovaGeolocation, Util) {
    ...
    if (Util.datosAuxiliar.realizarInsert) {
        ...
        $rootScope.db.transaction(function (tx) {
            ...
            for (i = 0; i < ServicioDatos.data.puntosCobro.length; i++) {
                tx.executeSql("INSERT INTO mapas(nombre, direccion, linea, telefono, latitud, logintud) VALUES (?,?,?,?,?,?)", [ServicioDatos.data.puntosCobro[i].nombre, ServicioDatos.data.puntosCobro[i].direccion,
    ...
    .controller('InfVencimientosCtrl', function ($scope, $ionicActionSheet, $timeout, $rootScope, ServiciosRuat, ServicioDatos, $location, $ionicLoading, $ionicPopup, $ionicHistory) {
    $scope.$on("$ionicView.enter", function () {
        $ionicHistory.clearCache();
        $ionicHistory.clearHistory();
    });
    ...
    $scope.diasRestantes = function (fechaFin, fechaActual) {
        ...
        var fechaActual = String(dia + "/" + mes + "/" + anio);
        var afechaActual = fechaActual.split('/');
    ...
    .controller('InfCargarMapasCtrl', function ($scope, $ionicActionSheet, $timeout, $rootScope, ServiciosRuat, ServicioDatos, $location, $ionicLoading, $ionicPopup, $ionicHistory, $state, $cordovaGeolocation, Util) {
    ...
    .controller('InfAcercadeCtrl', function ($scope, $ionicActionSheet, $timeout, $rootScope, ServiciosRuat, ServicioDatos, $location, $ionicLoading, $ionicPopup, $ionicHistory, $state, $sce) {
    try {
        if (cordova) {
    ...
    if (ServicioDatos.data) {
        ...
        $scope.urlDescarga = cadenaQR[3];
        var url = $scope.urlDescarga + "";
        var urlSplit = url.split("=");
        var dominio = urlSplit[0];
        $scope.claveComprobante = urlSplit[1];
        if (dominio.indexOf('.jsf') != -1) {
            $scope.showLoadingProperTimes();
            var urlModulo = '/verificarPago/obtieneRubro/' + $rootScope.globalExample + '/' + $scope.claveComprobante;
            $scope.resultado = ServiciosRuat.getGlobal(urlModulo);
            $scope.resultado.then(function (datas) {
                if (datas.status == 200 || datas.status == 401) {
                    $scope.hideLoadingProperTimes();
                    $scope.rubro = datas.data.rubro + "";
                    $scope.esComprobante = false;
                    if (dominio.indexOf('C.jsf') != -1) {
                        $scope.tipoPDF = 'Comprobante de Pago';
                        $scope.esComprobante = true;
                    }
                    if (dominio.indexOf('D.jsf') != -1) {
    ...
    $scope.abrirPdf = function () {
        //window.open($scope.urlDescarga, '_system', 'location=yes');
        var tipoImpresion;
        if ($scope.esComprobante) {
            tipoImpresion = 'CP'; //comprobantePago
        }
        else {
            tipoImpresion = 'DP'; //detallePago
        }
        $scope.datosURL = $rootScope.urlServicio + '/verificarPago/generarComprobantePDF/'
            + $rootScope.globalExample
            + '/' + $scope.claveComprobante
            + '/' + tipoImpresion
            + '/comprobantePago.pdf';
        $scope.hideLoadingProperTimes();
