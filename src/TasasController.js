...
angular.module('ControladorTasasOtrosIngresos', [])
    .controller('TsBusCtrl', function ($scope, $ionicActionSheet, $timeout, $rootScope, ServiciosRuat, ServicioDatos, $location, $ionicLoading, $ionicPopup, $ionicHistory, $state) {
    ...
    $scope.buscarTasa = function (numTasa, codAlcaldia, codTasa) {
        $scope.showLoadingProperTimes();
    ...
    .controller('TsDatosCtrl', function ($scope, $ionicActionSheet, $timeout, $rootScope, ServiciosRuat, ServicioDatos, $location, $ionicLoading, $ionicPopup, $ionicHistory, $state) {
    $scope.$on("$ionicView.enter", function () {
        $ionicHistory.clearCache();
        $ionicHistory.clearHistory();
    });
    ...
    $scope.resultado.then(function (datas) {
        $scope.hideLoadingProperTimes();
        var data = datas.data;
        if (datas.status == 401 || datas.data == 200) {
            try {
                $scope.tasaElegida = ServicioDatos.data.codTasa;
